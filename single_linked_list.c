// http://programmers.stackexchange.com/questions/128520/what-are-concrete-rules-for-using-a-linked-list-instead-of-an-array
// https://en.wikipedia.org/wiki/Linked_list#Related_data_structures
/*
 * C Program to Implement Singly Linked List using Dynamic Memory Allocation
 * Linked lists can also be used for the adjacency list representation of graphs.
...
snode* create_node(int val)
{
    ...
    if (newnode == NULL)
    {
        printf("\nMemory was not allocated. OR. You have gone out of memory");
        return 0;
    }
    else
    {
...
void insert_node_first()
{
    ...
    else
    {
        temp = first;
        first = newnode;
        first->next = temp;// here first->next = temp NOT temp->next
    }
    printf("\n----INSERTED----");    
}
...
void insert_node_last()
{
    ...
    {
    	//remove red. attach new end.  attach red to new end;
        last->next = newnode;
        last = newnode;
        last->next = NULL;
    }
